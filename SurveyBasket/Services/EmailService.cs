...
public class EmailService(IOptions<MailSettings> mailSettings, ILogger<EmailService> logger) : IEmailSender
{
    ...
    public async Task SendEmailAsync(string email, string subject, string htmlMessage)
    {
        ...
        {
            Sender = MailboxAddress.Parse(_mailSettings.Mail),      //Mail To Send From
            Subject = subject
        };

        message.To.Add(MailboxAddress.Parse(email));     //Mail To Send To

        var builder = new BodyBuilder    //Email Body
        {
